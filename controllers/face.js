...
exports.DeleteFace = async (req, res) => {
  try {
    var faceId = req.params.id;
    const { id } = await ConfidenceLevels.findOne({ title: "Nivel 3" });
    var update = {
      unknown: true,
      name: "",
      surname: "",
      age: 45,
      gender: "Femenino",
      image: "que-lo-que-manito-2022-06-02T21-25-34-03-00.jpg",
      confidenceLevels: id,
      user: "628d91118ad95d2f30032d0f",
    };
...
const subirArchivo = (files, extensionesValidas = ["jpg", "png"], name) => {
  return new Promise((resolve, reject) => {
    ...
    const nombreTemp =
      nombreCortado[0].replace(regex, "-") + "-" + name + "." + extension;
